...
const branches = [
  ...
  },
  {
    ...
    phone: "+254 722 940 203",
    inventoryValue: "312400",
    salesToday: "92700",
  },
] as const;
  ...
  try {
    ...
    const created = await db.transaction(async (tx) => {
      ...
      for (const item of body.items ?? []) {
        ...
        if (!stock) throw new Error(`Insufficient stock for product ${item.productId}.`);
      }
