...
4. **pnpm** - Package manager (install with: `npm install -g pnpm`)
...
2. Install dependencies:
   ...
   pnpm install
   ```
3. Initialize the database (this runs migrations and creates the admin user):
   ```powershell
...
npm install -g pm2
cd api-server
pm2 start "pnpm start" --name pajoy-api
pm2 save
pm2 startup
```
